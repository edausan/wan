...
import {NoteAlt, MusicNote, PlayArrow, CloseOutlined, Close, TextSnippet} from "@mui/icons-material"
...
const LineupCard = ({category, setLineupData, songs, songData, saving}) => {
	...
	const handleFilterSongs = useCallback(() => {
		if (songs?.length > 0 && category.id) {
			const filtered = songs?.filter(song => song?.tags[0] === category?.tags[0])
	...
	return (
		<Grid item xs={12} md={12}>
			<Modal open={open.status} onClose={() => setOpen({modal: null, status: false})}>
				{open.modal === "lyrics" ? (
					<Lyrics
			...
			<Card sx={{mb: 2}}>
				<CardContent>
					<FormGroup>
						<FormControlLabel
							control={<Switch checked={newSong} onChange={() => setNewSong(!newSong)} />}
							label="New"
						/>
					</FormGroup>
					...
					<Grid container sx={{mt: 1, width: "100%"}} spacing={1} justifyContent="left">
						<Grid item xs={6} md={6}>
							<TextField
								label="Artist"
								fullWidth
								...
								disabled={saving}
								onChange={e => setCardData({...cardData, artist: e.target.value})}
							/>
						</Grid>
						<Grid item xs={6} md={6}>
							<TextField
								...
								disabled={saving}
								onChange={e => setCardData({...cardData, album: e.target.value})}
							/>
						</Grid>
					</Grid>
				</CardContent>
				...
				<CardActions>
					<Button
						variant="text"
						color={songData?.lyrics?.verse || cardData.lyrics?.verse ? "primary" : "inherit"}
						disableElevation
						size="small"
						disabled={saving}
					...
					<Button
						variant="text"
						color={songData?.chords || cardData.chords ? "secondary" : "inherit"}
						disableElevation
						size="small"
						disabled={saving}
						sx={{minWidth: "40px !important"}}
