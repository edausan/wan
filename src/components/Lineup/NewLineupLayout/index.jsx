...
const NewLineupLayout = ({ edit }) => {
  ...
  useEffect(() => {
    const id = params?.id;
    if (edit && id) {
      const currentLineup = lineupsQuery.data?.filter((lineup) => lineup.id === id)[0];
  ...
  useEffect(() => {
    if (
      (mutatedNewLineup.isSuccess && !mutatedNewLineup.isLoading && !mutatedNewLineup.isError) ||
  ...
  return (
    <section className="flex flex-col items-center justify-between h-[calc(100vh-58px)] w-full max-w-[480px] mx-auto bg-white overflow-hidden">
      <LoadingScreen status={mutatedNewLineup.isLoading || mutatedLineup.isLoading} />
      ...
      <div className="flex flex-row items-start justify-between px-8 py-4 w-full border-b border-slate-100">
        ...
        <Step isCurrent={currentStep.step >= 2} label={`Review & ${edit ? "Update" : "Save"}`} />
      </div>
      ...
      <div className="flex flex-row items-end justify-between w-full p-8 py-4 border-t border-slate-100">
        ...
        />
        <ActionButton
          disabled={(currentStep.step === 1 && songs.length <= 0) || !service || !date}
          handleStep={currentStep.step >= 2 ? (edit ? handleUpdate : handleSave) : handleNext}
          icon={
            currentStep.step >= 2 ? (
...
const Step = ({ isCurrent, label }) => {
  return (
    <div className="relative w-[30%] flex flex-col gap-2 text-center">
      <small className={` ${isCurrent ? "text-sky-500" : "text-slate-400"}`}>{label}</small>
      <div
...
const ActionButton = ({ disabled, handleStep, icon, label, iconPosition = Position, hidden }) => {
  return (
    <button
      disabled={disabled}
      onClick={handleStep}
      className={`flex flex-row items-center justify-center gap-2 px-4 py-2 text-md rounded-full  disabled:bg-white disabled:text-slate-300 ${
        label === "Back" ? "bg-slate-100/30 text-slate-500" : "bg-green-600 text-white"
      }  `}
    >
      {!hidden && (
        <span>
