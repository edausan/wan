...
import { Alert, Box, Button, Snackbar, SwipeableDrawer, Tab } from "@mui/material";
...
import Chord from "@components/CustomComponents/Chord";

const SongDetailsDrawer = ({ expanded, handleExpandClick, handleClose, drawerData }) => {
    ...
    >
      ...
      <Box className="laptop:w-[500px] desktop:w-[500px] phone:w-[95vw] p-4  flex-1">
        ...
        <section className="flex flex-col gap-4">
          ...
          {drawerData?.song?.lyrics?.pre_chorus && drawerData.id === "Lyrics" && (
            <SongPart part="Pre-chorus" data={drawerData?.song?.lyrics?.pre_chorus} />
          )}
        </section>
                ...
                label={
                  <div className="flex flex-row items-center gap-2 text-xs">
                    <img
                      src={SPOTIFY_LOGO}
                      alt=""
                      className={`w-[25px] ${!media?.spotify ? "grayscale opacity-30" : ""}`}
                    />{" "}
                    Spotify
                  </div>
                }
