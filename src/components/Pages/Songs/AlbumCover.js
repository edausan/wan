  ...
  return (
    <SwipeableDrawer anchor="bottom" open={open} onClose={() => setOpen(false)}>
        ...
        <ImageList variant="masonry" cols={3} gap={6}>
          {albumCovers?.map((cover, idx) => (
            <ImageListItem
              ...
              className={`${selectedCover?.name === cover?.name ? "border border-sky-500" : ""}`}
            >
              <LazyLoadImage
                alt={cover?.name}
                effect="blur"
              ...
              {/* <img
                ...
                loading='lazy'
              /> */}
              <ImageListItemBar title={<span className="text-xs">{cover.name}</span>} />
            </ImageListItem>
          ))}
        </ImageList>
      </div>
      ...
      <div className="flex items-center justify-center">
        ...
        <button onClick={handleClick} className="p-2 text-sky-300 flex flex-row gap-2 items-center">
          <AddAPhotoOutlined />
          Upload Album Cover
        </button>
        {/* </label> */}
