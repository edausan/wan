...
const Service = ({
	...
	const user = auth.currentUser;
	const exist =
		user.uid === assignment.backups.filter((b) => b === user.uid)[0] ||
	...
	useEffect(() => {
		const complete =
			assignment.wl && assignment.backups.length > 0 && assignment.date;
							...
							<FormControl
								...
								<Select
									...
									renderValue={(selected) => {
										const v = via.filter(
											(v) => v.displayName && v.uid === selected
										)[0];
										return (
											<Chip
												key={selected}
