...
const SetAssignment = ({isViewing, assignment}) => {
	...
	const handleSetAssignments = async () => {
		...
		const assign = {
			...
			created_by: {uid: user.uid, photoURL: user.photoURL, displayName: user.displayName}
		}
	...
	return (
		<Card sx={{width: "100%", mb: !viewing ? 100 : 2}}>
			...
			{assignments.belleview.date && assignments.belleview.wl && assignments.belleview.backups.length > 0 && !viewing && (
				<>
					<Divider />
...
const Service = ({title, via, setVia, expand, isLumina, isYS, assignment, viewing, setAssignments, setViewing}) => {
	const theme = useTheme()
	const user = auth.currentUser
	const exist = user.uid === assignment.backups.filter(b => b === user.uid)[0] || user.uid === assignment.wl
	...
	return (
		<Accordion expanded={expanded} disableGutters sx={{boxShadow: "none", py: 0, background: "none"}}>
			<AccordionSummary
						...
						primary={
							isYS ? (
								<span style={{display: "flex", alignItems: "center"}}>
									{completed && !viewing && <CheckCircle sx={{fontSize: 16, mr: 1}} color="success" />} Youth Service
								</span>
							) : (
								<span style={{display: "flex", alignItems: "center"}}>
									{completed && !viewing && <CheckCircle sx={{fontSize: 16, mr: 1}} color="success" />} Worship Service
									| {title}
								</span>
							)
						}
							...
							>
								...
								>
									{via
										...
										.map(v => {
											return (
												<MenuItem key={v.uid} value={v.uid} disabled={v.is_wl || (isLumina && v.is_backup)}>
													<Avatar src={v.photoURL} sx={{width: 24, height: 24, mr: 1}} />
													{v.displayName || v.email}{" "}
													{v.is_wl && <MicTwoTone color="text.secondary" sx={{ml: 1, fontSize: 14}} />}
												</MenuItem>
											)
										})}
								</Select>
					...
					<ListItemText
						primary={
							...
							>
								...
								<Select multiple multiline value={viewing ? assignment.backups : backups} onChange={handleChange}>
									{via
										.filter(v => v.displayName)
										.map((v, i) => {
											return (
