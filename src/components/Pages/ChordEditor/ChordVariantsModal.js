...
import { CHORDS } from "../Songs/Chorder/Chords";
...
const ChordVariantsModal = ({ chord, setChord, open, replaceChord }) => {
	...
	const handleReplace = (selectedChord) => {
		...
		console.log({ selectedChord, part: chord.label });
		...
		<Dialog
			...
			<section className="w-[80vw] h-[80vh] overflow-hidden flex flex-col items-center justify-center">
				<div className="px-4 py-2 text-left w-full">
					Selected Chord: {chord.chord}
				</div>
				<div className="flex-1 flex flex-col gap-2 max-h-[100%] overflow-auto p-4">
					<Variants
						variant={variants?.maj}
						label="Major Variant"
						replaceChord={handleReplace}
					/>
					<Variants
						variant={variants?.min}
						label="Minor Variant"
						replaceChord={handleReplace}
					/>
					<Variants
				...
				<div className="p-4">
					<button onClick={() => setChord({ chord: null, label: null })}>
						Close
					</button>
				</div>
			</section>
