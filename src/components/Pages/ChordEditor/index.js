...
import { CHORDS, MUSICAL_NOTES } from "../Songs/Chorder/Chords";
...
const ChordEditor = () => {
	...
	const handleChangeKey = (e) => {
		...
		const family_chords = CHORDS.filter(
			(chord) => chord.root === e.target.value
		)[0];
	...
	const handleReplaceChord = ({ chord, part }) => {
		...
		const altered = selected_part.filter(
			(c) => (c.name ? c.name : c) !== replacing
		);
