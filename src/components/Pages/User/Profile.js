...
const Profile = () => {
	...
	const handleLineups = async () => {
		...
		setUserlineup(userLineups.sort((a, b) => new Date(b.date_created) - new Date(a.date_created)))
				...
				<CardContent sx={{pt: 0}}>
					<List>
						<ListItem>
							...
							<ListItemText
								primary={user?.displayName ? user?.displayName : user?.email || <Skeleton variant="text" />}
						...
						<ListItem>
							...
							<ListItemText
								primary={Ministries.filter(m => m.id === user?.ministry)[0]?.name || <Skeleton variant="text" />}
								secondary={"Ministry"}
							/>
						</ListItem>
						<ListItem>
							...
							<ListItemText
								...
								primary={user?.life_verse || <Skeleton variant="rectangular" width={210} height={118} />}
				...
				<CardContent>
					...
					<Grid container spacing={1} justifyContent="left" alignItems="stretch">
						{friends
							...
							.map(f => {
								return (
									<Grid item alignItems="stretch" display="flex" sx={{width: 75}}>
										{f.uid ? (
											<Link to={`/profile/${f.uid}`} style={{textDecoration: "none", color: "inherit", width: "100%"}}>
												<Card
													sx={{
												...
												>
													{f.photoURL ? (
														<CardMedia component="img" alt={f.displayName} src={f.photoURL} height={55} sx={{p: 0}} />
													) : (
													...
													>
														<Typography
															variant="caption"
															sx={{textOverflow: "ellipsis", width: 50, textAlign: "center", whiteSpace: "nowrap"}}
														>
															<div
																style={{
															...
															>
																{f.displayName ? f.displayName.split(" ")[0] : f.email.split("@gmail.com")[0]}
															</div>
														</Typography>
													</CardContent>
												</Card>
											...
											>
												...
												<CardContent>
													<Typography variant="caption" sx={{mt: 1}}>
														<div
															style={{textOverflow: "ellipsis", width: 37, whiteSpace: "nowrap", overflow: "hidden"}}
														>
															{f.displayName ? f.displayName.split(" ")[0] : f.email.split("@gmail.com")[0]}
														</div>
													</Typography>
            ...
            >
              ...
              >
                View Profile
						...
						<Link to="/edit_profile" style={{textDecoration: "none", width: "100%", flex: 1}}>
							<Button startIcon={<Edit />} size="small" fullWidth sx={{px: 2, py: 1}}>
								Edit Profile
							</Button>
						</Link>
					</CardActions>
	...
	return (
		<Box sx={{width: "100%"}}>
			...
			<TabPanel value={value} index={0}>
				{userlineup.length > 0 ? (
					userlineup.map((l, i) => {
						return <LineupItem key={l.id} lineup={l} isBordered isLast={userlineup.length - 1 === i} />
					})
				) : (
