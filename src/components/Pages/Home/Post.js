...
const Post = ({ post: current, profile }) => {
  ...
  useEffect(() => {
    ...
    } else if (params?.id !== currentUser.uid || (!profile && postsQuery.data)) {
      handleGetPost();
    }
  ...
  const handleReact = async () => {
    try {
      const idx = post?.reactions?.findIndex((r) => r.uid === userProfile.uid) === -1;
      const reacts =
        idx === -1 ? post?.reactions : post?.reactions.filter((r) => r.uid !== userProfile.uid);
      const reactions = [
        ...reacts,
        {
          uid: userProfile.uid,
  ...
  const handleReactions = (reaction, list) => {
    switch (reaction) {
      ...
      case "sad":
        return {
          icon: <SentimentVeryDissatisfiedTwoTone className="text-purple-500" />,
  ...
  return (
    <Card className={`w-full shadow-md ${!profile ? "max-w-[680px] mx-auto" : ""}`} elevation={0}>
      <ReactionsModal
      ...
      <CardHeader
        ...
        subheader={
          <span className="text-xs">
            {post?.date_created ? post?.date_created : <Skeleton variant="text" />}
          </span>
        }
      />
      ...
      <CardContent>
        ...
        <div>
          {hashtags?.map((hash, idx) => {
            return (
              <Link key={`${hash}~${idx}`} to="#" className="no-underline text-sky-500 italic">
                #{hash}
              </Link>
            );
          })}
      ...
      <CardActions className="justify-center grid grid-cols-3 p-1">
        <Button
          ...
          startIcon={
            post?.reactions?.findIndex((r) => r?.uid === userProfile?.uid) !== -1 || reaction ? (
              handleReactions(
                reaction ||
                  post?.reactions?.filter((r) => r?.uid === userProfile?.uid)[0]?.reaction,
              )?.icon
            ) : (
          ...
          >
            {reaction?.charAt(0).toUpperCase() + reaction?.slice(1) ||
              post?.reactions
                ...
                post?.reactions
                  ?.filter((r) => r?.uid === userProfile?.uid)[0]
                  ?.reaction?.slice(1) || <span className="text-white/40">Like</span>}
          </span>
        </Button>
        <Button
          variant="text"
          className={`!ml-0 ${post?.comments?.length > 0 ? "text-sky-500" : ""} col-span-1`}
          startIcon={<CommentTwoTone className="w-[16px] h-[16px]" />}
          onClick={() => setOpenComments(true)}
        >
          <span className="!capitalize text-xs">
            Comment
            {post?.comments?.length > 1 ? <span className="lowercase">s</span> : ""}{" "}
        ...
        >
          <a
            ...
            onClick={() => {
              navigator.clipboard.writeText(`https://wan-belleview.web.app/post/${post?.id}`);
            }}
            className="ml-2"
          >
      ...
      {post?.comments?.length > 0 && (
        <>
          ...
          <CardContent className="!pb-1 p-1">
            <PostComments open={openComments} setOpen={setOpenComments} post={post} preview />
          </CardContent>
        </>
      )}
      ...
      <ReactionDrawer
        ...
        reaction={
          reaction || post?.reactions?.filter((r) => r?.uid === userProfile?.uid)[0]?.reaction
        }
        post={post}
      />
