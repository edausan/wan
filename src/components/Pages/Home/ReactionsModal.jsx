...
const ReactionsModal = ({ reactions, friends, setOpen, open, handleReactions }) => {
  return (
    <Modal open={open} onClose={() => setOpen(false)}>
      <Card className="p-2 absolute top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%] w-[250px]">
        <CardHeader className="py-0" title={<span className="text-sm">Reactions</span>} />
              ...
              return (
                <Link key={react?.uid} to={`/profile/${user?.uid}`}>
                  <ListItemButton className="px-2">
                    <ListItemIcon
                      className={`${handleReactions(react.reaction)?.color.split("text")[1]}`}
                    >
                      {handleReactions(react.reaction)?.icon}
                    </ListItemIcon>
                    <ListItemText primary={<span className="text-sm">{user?.displayName}</span>} />
                  </ListItemButton>
                </Link>
              );
            })}
