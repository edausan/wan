...
const Login = ({setScreen}) => {
	...
	return (
		<>
			<IconButton onClick={() => setMode(!mode)} sx={{position: "fixed", left: 20, bottom: 20, zIndex: 1002}}>
			...
			>
				...
				{error && (
					<Alert severity="error" sx={{mb: 1, textAlign: "left", fontSize: 12}} onClose={() => setError(null)}>
						{error}
					</Alert>
				)}
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={12} md={12} sx={{mt: 2}}>
						<Grid container spacing={2}>
							...
							<Grid item xs={4} md={4}>
								<Button variant="text" fullWidth disabled={isSigning} onClick={() => setScreen("signup")}>
									Sign Up
								</Button>
							</Grid>
						</Grid>
